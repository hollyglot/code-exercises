...
main_title 'Find Least Aligned CCSS Math Standards'
...
Benchmark.bm do |x|
  x.report do
    ...
    puts "\n====================================\n\n"
    puts "Getting CCSS Alignment Templates...\n\n"

    standard = Standard.where(name: "CCSS").first
    ccss_alignment_templates = AlignmentTemplate.all_of(:standard_id => standard.id, :subject => "Math").asc(:grade)

    ccss_alignment_templates.each do |at|
      puts "\n\nAlignment Template"
      puts at.title
